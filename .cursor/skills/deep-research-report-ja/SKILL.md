...
統合レポートはリポジトリ直下の **`research/YYYY-MM-DD-<slug>.md`** に書く。

- `knowledge/` には置かない（`knowledge/` は実装意図・インシデント用）
- 閲覧は `apps/report-viewer`（ポータルの Research 枠）。変更後は当該アプリを `npm run build` する
- README など案内ファイルはレポート一覧の対象外
