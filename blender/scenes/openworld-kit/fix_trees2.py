...
def rebuild_tree(obj, kind):
    ...
    # trunk: explicit from z=0 to z=h*0.62
    trunk_h = h * 0.62
    s=len(bm.verts); f0=len(bm.faces)
    bmesh.ops.create_cone(bm, cap_ends=True, segments=8, radius1=r0, radius2=r0*0.55, depth=trunk_h)
    for v in bm.verts[s:]:
        v.co.z += trunk_h * 0.5
    # force base exactly 0
    zmin=min(v.co.z for v in bm.verts)
    for v in bm.verts:
        v.co.z -= zmin
    bm.faces.ensure_lookup_table()
    for i in range(f0, len(bm.faces)):
        bm.faces[i].material_index = bark_i
    trunk_top = max(v.co.z for v in bm.verts)

    if kind=="pine":
        for i,t in enumerate((0.08,0.28,0.48,0.68)):
            s=len(bm.verts); f0=len(bm.faces)
            rad=1.5-i*0.28; depth=1.25-i*0.12
            bmesh.ops.create_cone(bm, cap_ends=True, segments=8, radius1=rad, radius2=0.05, depth=depth)
            for v in bm.verts[s:]:
                v.co.z += trunk_top + t * (h-trunk_top) + depth*0.45
            bm.faces.ensure_lookup_table()
            for j in range(f0,len(bm.faces)):
                bm.faces[j].material_index = leaf_i
    elif kind=="dead":
        for ang,zh,length in ((0.5,0.5,1.2),(2.4,0.68,1.0),(4.0,0.58,0.9)):
            s=len(bm.verts); f0=len(bm.faces)
            ...
            bm.faces.ensure_lookup_table()
            for j in range(f0,len(bm.faces)):
                bm.faces[j].material_index = bark_i
    else:
        clumps={
        ...
        for x,y,rad in clumps:
            ...
            # place fully above trunk_top
            center=Vector((x,y, trunk_top + rad*0.85))
            for v in bm.verts[s:]:
                v.co.z *= 0.7
                v.co *= 1.0 + noise.noise(v.co*2.0+center)*0.12
                v.co += center
            bm.faces.ensure_lookup_table()
            for j in range(f0,len(bm.faces)):
                bm.faces[j].material_index = leaf_i

    # center XY only; keep z grounded at 0 based on bark base
    ...
    # materials already on object; ensure slots
    while len(mesh.materials)< (1 if kind=="dead" else 2):
        mesh.materials.append(None)
    # remap names
    bark = bpy.data.materials.get("M_bark_light" if kind=="birch" else "M_bark")
    leaf = {"oak":"M_leaf","pine":"M_pine","birch":"M_leaf_yellow","autumn":"M_autumn","dead":None}[kind]
    mesh.materials[0]=bark
    if leaf:
        if len(mesh.materials)<2: mesh.materials.append(bpy.data.materials.get(leaf))
        else: mesh.materials[1]=bpy.data.materials.get(leaf)
...
print("oak bark", round(min(zb),2), round(max(zb),2), "leaf", round(min(zl),2), round(max(zl),2), "dims", tuple(round(x,2) for x in o.dimensions))
