/** Particle beach helpers. */

export function clampDt(dt: number, maxDt = 1 / 45): number {
  if (!Number.isFinite(dt) || dt < 0) return 0;
  return dt > maxDt ? maxDt : dt;
}

export function shouldBreak(
  depth: number,
  speed: number,
  crest: number,
  foamGain: number,
): boolean {
  if (depth <= 0 || depth >= 0.07) return false;
  if (crest < 0.3 || speed < 0.2) return false;
  return foamGain > 0;
}

export function foamLifeDecay(life: number, dt: number, onSand: boolean): number {
  const rate = onSand ? 1.5 : 1;
  return life - dt * rate;
}

export function screenToWaterUv(
  ndcX: number,
  ndcY: number,
  invViewProj: Float32Array,
): { u: number; v: number } | null {
  const near = unproject(ndcX, ndcY, -1, invViewProj);
  const far = unproject(ndcX, ndcY, 1, invViewProj);
  const dx = far.x - near.x;
  const dy = far.y - near.y;
  const dz = far.z - near.z;
  if (Math.abs(dy) < 1e-6) return null;
  const t = -near.y / dy;
  if (t < 0 || t > 1) return null;
  const x = near.x + dx * t;
  const z = near.z + dz * t;
  const u = x + 0.5;
  const v = z + 0.5;
  if (u < 0 || u > 1 || v < 0 || v > 1) return null;
  return { u, v };
}

function unproject(
  ndcX: number,
  ndcY: number,
  ndcZ: number,
  inv: Float32Array,
): { x: number; y: number; z: number } {
  const x = inv[0] * ndcX + inv[4] * ndcY + inv[8] * ndcZ + inv[12];
  const y = inv[1] * ndcX + inv[5] * ndcY + inv[9] * ndcZ + inv[13];
  const z = inv[2] * ndcX + inv[6] * ndcY + inv[10] * ndcZ + inv[14];
  const w = inv[3] * ndcX + inv[7] * ndcY + inv[11] * ndcZ + inv[15];
  const iw = 1 / w;
  return { x: x * iw, y: y * iw, z: z * iw };
}

export function invert4(m: Float32Array): Float32Array | null {
  const out = new Float32Array(16);
  const a00 = m[0], a01 = m[1], a02 = m[2], a03 = m[3];
  const a10 = m[4], a11 = m[5], a12 = m[6], a13 = m[7];
  const a20 = m[8], a21 = m[9], a22 = m[10], a23 = m[11];
  const a30 = m[12], a31 = m[13], a32 = m[14], a33 = m[15];

  const b00 = a00 * a11 - a01 * a10;
  const b01 = a00 * a12 - a02 * a10;
  const b02 = a00 * a13 - a03 * a10;
  const b03 = a01 * a12 - a02 * a11;
  const b04 = a01 * a13 - a03 * a11;
  const b05 = a02 * a13 - a03 * a12;
  const b06 = a20 * a31 - a21 * a30;
  const b07 = a20 * a32 - a22 * a30;
  const b08 = a20 * a33 - a23 * a30;
  const b09 = a21 * a32 - a22 * a31;
  const b10 = a21 * a33 - a23 * a31;
  const b11 = a22 * a33 - a23 * a32;

  let det =
    b00 * b11 - b01 * b10 + b02 * b09 + b03 * b08 - b04 * b07 + b05 * b06;
  if (Math.abs(det) < 1e-8) return null;
  det = 1 / det;

  out[0] = (a11 * b11 - a12 * b10 + a13 * b09) * det;
  out[1] = (a02 * b10 - a01 * b11 - a03 * b09) * det;
  out[2] = (a31 * b05 - a32 * b04 + a33 * b03) * det;
  out[3] = (a22 * b04 - a21 * b05 - a23 * b03) * det;
  out[4] = (a12 * b08 - a10 * b11 - a13 * b07) * det;
  out[5] = (a00 * b11 - a02 * b08 + a03 * b07) * det;
  out[6] = (a32 * b02 - a30 * b05 - a33 * b01) * det;
  out[7] = (a20 * b05 - a22 * b02 + a23 * b01) * det;
  out[8] = (a10 * b10 - a11 * b08 + a13 * b06) * det;
  out[9] = (a01 * b08 - a00 * b10 - a03 * b06) * det;
  out[10] = (a30 * b04 - a31 * b02 + a33 * b00) * det;
  out[11] = (a21 * b02 - a20 * b04 - a23 * b00) * det;
  out[12] = (a11 * b07 - a10 * b09 - a12 * b06) * det;
  out[13] = (a00 * b09 - a01 * b07 + a02 * b06) * det;
  out[14] = (a31 * b01 - a30 * b03 - a32 * b00) * det;
  out[15] = (a20 * b03 - a21 * b01 + a22 * b00) * det;
  return out;
}
