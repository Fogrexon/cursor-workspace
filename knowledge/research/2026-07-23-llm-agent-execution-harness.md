...
| 7 | ThunderAgent | ICML 2026 Spotlight | 3/3 | 査読済み、引用は未蓄積 |
...
| ThunderAgent | Agentic Program | KV、phase、placement、tool environment | LLM backend + orchestrator。security非主題 | pause/restore、resource GC | 8〜64 H100、複数agent | ICML Spotlight。高並列self-host前提 |
...
終端stateは変更しない。再実行は新しい`run_id`と`parent_run_id`を作る。上図を実装する際は、次の遷移も明示する。

| 発生箇所 | timeout / failure時の遷移 |
|---|---|
| `PROVISIONING` | retry可能なら新`attempt_id`で再provision。上限後`FAILED_INFRA` |
| `WAITING_TOOL` | effectなしが確定すればbounded retry。effect不明なら`UNKNOWN_EXTERNAL_EFFECT` |
| `WAITING_APPROVAL` |期限切れで`APPROVAL_EXPIRED`。引数変更時は再承認 |
| `CHECKPOINTING` | 古いcheckpointを保持してretry。整合点を作れなければrunを進めない |
| `VERIFYING` | grader infra失敗はagent failureと分離してretry。functional failureは`REPAIRING` |
| 任意の非終端state | cancel requestを受け、実行中toolを停止またはeffect確認して`CANCELLED` |

状態更新とevent appendは同じtransaction、またはtransactional outboxで原子的に扱う。workerはleaseと単調増加するfencing tokenを持ち、期限切れworkerが古いcommandをcommitできないようにする。retryは同じstepを上書きせず、新しい`attempt_id`を作る。非冪等toolがtimeoutした場合は、同じcallを再送する前に`idempotency_key`でstatusを照会する。
...
| 22 | [ThunderAgent](https://arxiv.org/abs/2602.13692) | 2026-02 | 主 | 3/3/3/3/2/3/2 | workflow-level serving、ICML Spotlight |
...
| 29 | Durable Execution for AI Agents | 2026-05 | 補 | 3/3/2/3/1/2/2 | durable workflow直接、公開・追試蓄積前 |
...
7. Kang, H. et al. “ThunderAgent: A Fast, Simple, and Program-Aware Agentic Inference System.” ICML 2026 Spotlight. [原典](https://arxiv.org/abs/2602.13692)
